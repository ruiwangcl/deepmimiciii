...
def icd_to_comorbidities(df, idx, icds, mapping="quan_elixhauser9"):
    """
    Summary:
    ...
    """
    validMappings = ["quan_elixhauser10", "charlson10","quan_elixhauser9"]
    ...
    for comorb in comorb_cols:
        ...
        truth_list = [[any([m for m in [x] if any(m.startswith(s) for s in mapping[comorb])]) for x in df[idx]] for idx in icds]
